...
const props = defineProps({
  ...
  winner: {
    type: Object as PropType<WinnerEvent>,
    default: null,
  }
})
...
const formSchema = z.object({
  tag: z.string().min(1, { message: 'Please enter a tag' }),
...
watch([() => props.winner, () => props.open], ([newWinner, isOpen]) => {
  if (newWinner && isOpen) {
    setFieldValue('tag', newWinner.tag)
    setFieldValue('date', newWinner.created_at)

    participants.value = (newWinner.participants as unknown as Array<Participant>).map((participant: Participant) => ({
      id: participant.id,
...
const onSubmit = handleSubmit(async (values) => {
  ...
  const { error } = await supabase.from('winners').update({
    ...
    tag,
    created_at: date ?? moment().toISOString(),
    participants: participants.value.map(({ id, name, enabled }) => ({
      id,
      name,
      enabled,
    })),
  }).eq('id', props.winner.id)
  ...
  toast.success('Winners Updated', {
    description: 'Winners have been updated.',
  })
